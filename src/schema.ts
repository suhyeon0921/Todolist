...
const resolvers = {
  ...
  Mutation: {
    ...
    deleteTask: async (_parent: any, args: any, context: any) => {
      ...
      if (!task) {
        throw new GraphQLError(
          '이미 삭제된 태스크이거나 해당 태스크를 작성한 유저가 아닙니다.'
        );
      }

      const deletedTask = await context.prisma.task.update({
        where: { id: Number(id) },
      ...
      return {
        ...
        deletedAt: deletedTask.deletedAt
          ? deletedTask.deletedAt.toISOString()
          : null,
      };
    },
    completeTask: async (_parent: any, args: any, context: any) => {
      ...
      const doneTask = await context.prisma.task.update({
        where: { id: Number(id) },
        data: { isDone: true },
        include: { user: true },
      });
