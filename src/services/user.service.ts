...
class UserService {
  ...
  public async checkExistingUser(
    ...
    const existingUser: User | null = await userRepository.findUser({
      email,
      phoneNumber,
      nickname,
    });
  ...
  /** 유저 생성 */
  public async signup(args: SignupInput): Promise<User> {
  ...
  public async refreshAccessToken(refreshToken: string): Promise<JwtToken> {
    try {
      ...
      const storedUser: User | null =
        await userRepository.findUserWithRefreshToken(
          payload.userId,
          refreshToken
        );
